...
function seedData() {
  const courses = [
    { id: "c1", name: "Desarrollo Web Full Stack", subjects: [
      { id: "s1", name: "Frontend Avanzado", teacherId: "t1" },
      { id: "s2", name: "Backend con Node.js", teacherId: "t2" },
    ]},
    { id: "c2", name: "Diseño UX/UI", subjects: [
      { id: "s3", name: "Investigación de Usuarios", teacherId: "t2" },
      { id: "s4", name: "Prototipado", teacherId: "t1" },
    ]},
  ];

  const teachers = [
    { id: "t1", name: "Marta Rovira" },
    { id: "t2", name: "David Ferrer" },
  ];

  const students = [
    { id: "a1", name: "Laia Puig", courseId: "c1" },
    { id: "a2", name: "Jordi Camps", courseId: "c1" },
    { id: "a3", name: "Nuria Soler", courseId: "c1" },
    { id: "a4", name: "Marc Bosch", courseId: "c2" },
    { id: "a5", name: "Elena Ferran", courseId: "c2" },
  ];

  const codes = [
    { code: "STAFF-2026", role: "staff", name: "Administración CESI", linkedId: null },
    { code: "PROF-ROVIRA", role: "profesor", name: "Marta Rovira", linkedId: "t1" },
    { code: "PROF-FERRER", role: "profesor", name: "David Ferrer", linkedId: "t2" },
    { code: "ALU-PUIG01", role: "alumno", name: "Laia Puig", linkedId: "a1" },
    { code: "ALU-CAMPS01", role: "alumno", name: "Jordi Camps", linkedId: "a2" },
    { code: "ALU-SOLER01", role: "alumno", name: "Nuria Soler", linkedId: "a3" },
    { code: "ALU-BOSCH01", role: "alumno", name: "Marc Bosch", linkedId: "a4" },
    { code: "ALU-FERRAN01", role: "alumno", name: "Elena Ferran", linkedId: "a5" },
  ];

  const today = new Date();
  const fmt = (d) => d.toISOString().slice(0, 10);
  const attendance = [];
  ["s1", "s2", "s3", "s4"].forEach((subjectId) => {
    students
      .filter((s) => courses.find((c) => c.id === s.courseId).subjects.some((sub) => sub.id === subjectId))
      .forEach((s) => {
        for (let i = 6; i >= 1; i--) {
          ...
          attendance.push({
            id: `att-${s.id}-${subjectId}-${i}`,
            studentId: s.id,
            subjectId,
            date: fmt(d),
            status: roll > 0.82 ? "ausente" : roll > 0.7 ? "tarde" : "presente",
          });
        }
      });
  });
  ...
  students.forEach((s) => {
    ...
    course.subjects.forEach((sub) => {
      grades.push({
        ...
        studentId: s.id,
        subjectId: sub.id,
        practica: Math.floor(5 + Math.random() * 5),
        examen: Math.floor(4 + Math.random() * 6),
        proyecto: Math.floor(5 + Math.random() * 5),
      });
    });
  });
  ...
  return {
    courses, teachers, students, codes, attendance, grades, certificates: [], templates,
    clockRecords: [],
    classSessions: [],
    activityLog: [
...
/* ============================================================
   LOGIN
   ============================================================ */
function LoginScreen({ onLogin, onGoVerify }) {
  ...
  return (
    <div className="login-wrap">
      <div className="login-card">
        ...
        <p className="login-sub">Portal académico · Acceso por código</p>
        ...
        <button type="button" className="btn btn-brass btn-full" style={{ marginTop: 18 }} disabled={loading} onClick={submit}>
          {loading ? "Verificando…" : <>Entrar <Lock size={14} /></>}
        </button>

        <div className="login-hint">
          Tu código determina tu rol (Alumno, Profesor o Staff) automáticamente.<br />
          Los códigos los emite exclusivamente el equipo de Staff.
        </div>
...
const NAV = {
  ...
  staff: [
    ...
    { key: "codigos", label: "Gestión de códigos", icon: Key },
    { key: "certificados", label: "Certificados", icon: Award },
  ],
};
              ...
              <div key={s.id} className="card clickable-row" onClick={() => setSubjectModal(s)}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    ...
                    <p style={{ margin: 0, fontSize: 12.5, color: "var(--ink-2)" }}>Docente: {teacher?.name}</p>
                  </div>
                  <ChevronRight size={16} color="var(--brass)" />
                </div>
              </div>
...
function StaffCodigos({ data, createCode, deleteCode }) {
  ...
  return (
    <>
      <div className="toolbar" style={{ justifyContent: "space-between" }}>
        ...
        <button className="btn btn-brass" onClick={openModal}><Plus size={14} /> Nuevo código</button>
      </div>
      ...
      <div className="table-wrap">
        <table className="data-table">
          ...
          <tbody>
            ...
            {filtered.length === 0 && <tr><td colSpan={4}><div className="empty-state"><p>Sin resultados.</p></div></td></tr>}
          </tbody>
        </table>
      </div>
...
export default function App() {
  ...
  // Sincronización en vivo entre sesiones (profesor / alumno / staff pueden estar conectados
  // en pestañas o dispositivos distintos). Sin esto, el estado del Meet (activo / finalizado)
  // no se reflejaría en tiempo real para quien no lo generó.
  useEffect(() => {
    if (route !== "app") return;
    let cancelled = false;
    const poll = async () => {
  ...
  if (route === "login" || !user) {
    return (
      <div className="acad-root paper-texture">
        <GlobalStyle />
        <LoginScreen onLogin={handleLogin} onGoVerify={() => setRoute("verify")} />
      </div>
    );
  }
  ...
  const titleMap = {
    ...
    "clase-en-vivo": "Clase en vivo", dashboard: "Panel principal", explorador: "Cursos, asignaturas y clases", codigos: "Gestión de códigos",
  };
  ...
  if (user.role === "alumno") {
    ...
    if (view === "explorador") content = <StaffExplorador data={data} />;
    if (view === "codigos") content = <StaffCodigos data={data} createCode={createCode} deleteCode={deleteCode} />;
